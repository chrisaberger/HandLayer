...
struct LSTM {
  ...
  LSTM(const int input_size, const int hidden_size, const int num_layers = 1,
    ...
    */
      ...
      std::cout << input_copy_size << std::endl;
      ...
      for (size_t i = 0; i < input_size; ++i) {
        for (size_t j = 0; j < hidden_size; ++j) {
          weights_l[i * hidden_size + j] = weight_ih_l_data.data()[j*input_size + i];
          weights_l[input_offset + i * hidden_size + j] = weight_ih_l_data.data()[input_copy_size + j*input_size + i];
          weights_l[2*input_offset + i * hidden_size + j] = weight_ih_l_data.data()[2*input_copy_size + j*input_size + i];
          weights_l[3*input_offset + i * hidden_size + j] = weight_ih_l_data.data()[3*input_copy_size + j*input_size + i];
        }
      }
      for (size_t i = 0; i < hidden_size; ++i) {
        for (size_t j = 0; j < hidden_size; ++j) {
          weights_l[input_copy_size+ i * hidden_size + j] = weight_hh_l_data.data()[j*hidden_size + i];
          weights_l[input_copy_size + input_offset + i * hidden_size + j] = weight_hh_l_data.data()[hidden_copy_size + j*hidden_size + i];
          weights_l[input_copy_size + 2*input_offset + i * hidden_size + j] = weight_hh_l_data.data()[2*hidden_copy_size + j*hidden_size + i];
          weights_l[input_copy_size + 3*input_offset + i * hidden_size + j] = weight_hh_l_data.data()[3*hidden_copy_size + j*hidden_size + i];
        }
      }

      print_matrix(weights_l, 4*(hidden_size+input_size), hidden_size);
  ...
  void forward(float* X, float * H, float * C, int batch_size){
    float* f_t = (float*)malloc(sizeof(float) * batch_size * hidden_size);
    memset(f_t, 0, sizeof(float) * batch_size * hidden_size);

    float* i_t = (float*)malloc(sizeof(float) * batch_size * hidden_size);
    memset(i_t, 0, sizeof(float) * batch_size * hidden_size);
    float* g_t = (float*)malloc(sizeof(float) * batch_size * hidden_size);
    memset(g_t, 0, sizeof(float) * batch_size * hidden_size);
    float* o_t = (float*)malloc(sizeof(float) * batch_size * hidden_size);
    memset(o_t, 0, sizeof(float) * batch_size * hidden_size);
    ...
    std::cout << "Running layers..." << std::endl;

    const size_t weights_index = (input_size+hidden_size)*hidden_size;
    print_matrix(&weights_l[weights_index], input_size+hidden_size, hidden_size);

    layer(x_h_in_cat, &weights_l[weights_index * 0], bias_ih_l, batch_size,
          hidden_size, (input_size + hidden_size), i_t, sigmoid);
    std::cout << "i_T" << std::endl;
    print_matrix(i_t, batch_size, hidden_size);

    layer(x_h_in_cat, &weights_l[weights_index * 1],
          bias_ih_l, batch_size, hidden_size, (input_size + hidden_size), f_t,
          sigmoid);

    layer(x_h_in_cat, &weights_l[weights_index * 2],
          bias_ih_l, batch_size, hidden_size, (input_size + hidden_size), g_t,
          [](float a) { return tanh(a); });

    layer(x_h_in_cat, &weights_l[weights_index * 3],
          bias_ih_l, batch_size, hidden_size, (input_size + hidden_size), o_t,
          sigmoid);


    //print_matrix(o_t, batch_size, hidden_size);
    ...
    print_matrix(h_t, batch_size, hidden_size);
    std::cout << "c_t" << std::endl;
    print_matrix(c_t, batch_size, hidden_size);
